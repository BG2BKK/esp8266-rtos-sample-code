...
LOCAL void websocket_task(void *pvParameters)
{
	...
	int          iter = 0;
	int 		 length = 0;
	const char	 *text_msg = "{\"path\": \"/v1/mbox/\", \"method\": \"POST\", \"body\": {\"action\": \"subscribe\", \"type\": \"datastream\", \"stream\": \"tem_hum\"}, \"meta\": {\"Authorization\": \"token ccf61f9f19fa15d9aba14084ba6c44af7ef70620\"}}\r\n";
#if PING_ALIVE
	...
	while(1){
		...
		switch (pwebinfo->WebOption){
			case WEBSOCKET_NORMAL:
				conn = nopoll_conn_new (ctx, pwebinfo->WebHostIp, pwebinfo->WebPort, pwebinfo->WebHostName, pwebinfo->WebUrl, pwebinfo->WebProtocols, pwebinfo->WebOrigin);
			...
			case WEBSOCKET_TLS:
				...
				/* call to create a connection */
				conn = nopoll_conn_tls_new (ctx, opts, pwebinfo->WebHostIp, pwebinfo->WebPort, pwebinfo->WebHostName, pwebinfo->WebUrl, pwebinfo->WebProtocols, pwebinfo->WebOrigin);
				if (! nopoll_conn_is_ok (conn)) {
					printf ("ERROR: Expected to find proper client connection status, but found error..\n");
					goto context_error;
				} else
